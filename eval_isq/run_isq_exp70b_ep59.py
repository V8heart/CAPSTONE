#!/usr/bin/env python3
"""ISQ evaluation for exp70b using ep0059_model.pth (not stale best_model.pth)."""
...
PKL_GNN = os.path.join(EVAL_ISQ_DIR, "pred_gnn_exp70b_ep59.pkl")
OUT_JSON = os.path.join(EVAL_ISQ_DIR, "results", "isq_results_exp70b_ep59.json")
OUT_MD = os.path.join(EVAL_ISQ_DIR, "results", "isq_summary_exp70b_ep59.md")
...
def _write_summary_md(results: dict) -> None:
    baselines = {
        "best_model": _load_baseline_summary(
            os.path.join(EVAL_ISQ_DIR, "results", "isq_results_exp70b.json"),
            "Pred-2_gnn_exp70b",
        ),
    ...
    s_gnn = results["Pred-2_gnn_exp70b_ep59"]["summary"]
    ...
    lines = [
        ...
        "- **Checkpoint**: `%s` (epoch 59, **not** stale `best_model.pth`)" % settings["exp70b_checkpoint"],
        "- **GNN thresholds**: edge=%.1f, node_conf=%.1f" % (settings["gnn_edge_thresh"], settings["gnn_node_conf"]),
        "",
        "## Summary",
        "",
        ...
        "| **Pred-2 GNN (exp70b ep59)** | **%.4f** | **%.4f** | **%.4f** | **%.2f** | **%.2f** |"
        % (s_gnn["precision"], s_gnn["recall"], s_gnn["f1"], s_gnn["over_split"], s_gnn["under_merge"]),
    ]
    if baselines["best_model"]:
        b = baselines["best_model"]
        lines.append(
            "| Pred-2 GNN (exp70b best_model, ref) | %.4f | %.4f | %.4f | %.2f | %.2f |"
...
def main() -> None:
    ...
    print("[INFO] checkpoint:", EXP70B_CKPT)
    for p in (EXP70B_CONFIG, EXP70B_CKPT, PKL_GEOM, dataset_root):
        if not os.path.exists(p):
            raise FileNotFoundError(p)

    with open(PKL_GEOM, "rb") as f:
        geom_pkl = pickle.load(f)
    print("[geom] reuse", PKL_GEOM)

    records = load_records(dataset_root, stems)
    gnn_raw: dict = {}
    t0 = time.time()
    print("[exp70b ep59] GNN inference (%d images)..." % len(stems))
    ...
    print("      done in %.1fs -> %s" % (time.time() - t0, PKL_GNN))
    ...
    results = {
        "settings": {
            ...
            "checkpoint_note": "ep0059_model.pth (epoch 59, not best_model.pth)",
        ...
        "Pred-2_gnn_exp70b_ep59": _eval_isq(isq_meta, gnn_pkl),
    }
    print("      ISQ done in %.1fs" % (time.time() - t1))

    for label, block in (
        ("Pred-1 geom (exp19)", results["Pred-1_geom_exp19"]),
        ("Pred-2 GNN (exp70b ep59)", results["Pred-2_gnn_exp70b_ep59"]),
    ):
        s = block["summary"]
        print(
