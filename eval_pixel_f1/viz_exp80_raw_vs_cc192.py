#!/usr/bin/env python3
"""4-panel viz: RGB | raw geom | CC adj=768 | CC adj=192 (+ GT overlay)."""
...
CC768_PKL = os.path.join(BASE, "eval_isq", "pred_geom_exp80_512_test.pkl")
CC192_PKL = os.path.join(BASE, "eval_isq", "pred_geom_exp80_512_test_adj192.pkl")
OUT_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "results", "viz_exp80_raw_vs_cc192"
)
...
def _pred_panel(rgb: np.ndarray, polylines, segments, title: str) -> np.ndarray:
    panel = rgb.copy()
    _draw_gt(panel, polylines)
    if segments and len(segments[0]) >= 5:
        _draw_cc_polylines(panel, segments)
    else:
        _draw_raw_segments(panel, segments)
    return _title_panel(panel, title)
...
def main() -> None:
    ...
    with open(CC768_PKL, "rb") as f:
        cc768_pkl = pickle.load(f)
    with open(CC192_PKL, "rb") as f:
        cc192_pkl = pickle.load(f)
    ...
    for stem in SELECTED_STEMS:
        ...
        cc768_segs = cc768_pkl[stem]
        cc192_segs = cc192_pkl[stem]
        n_raw = len(raw_segs)
        n_cc768_poly = len(set(int(s[4]) for s in cc768_segs))
        n_cc192_poly = len(set(int(s[4]) for s in cc192_segs))

        panel_rgb = _title_panel(rgb, "RGB")

        panel_raw = _pred_panel(
            rgb,
            polylines,
            raw_segs,
            "Raw (n=%d segs) + GT green" % n_raw,
        )
        panel_cc768 = _pred_panel(
            rgb,
            polylines,
            cc768_segs,
            "CC adj=768 (n=%d wires, %d segs)" % (n_cc768_poly, len(cc768_segs)),
        )
        panel_cc192 = _pred_panel(
            rgb,
            polylines,
            cc192_segs,
            "CC adj=192 (n=%d wires, %d segs)" % (n_cc192_poly, len(cc192_segs)),
        )

        row = np.concatenate([panel_rgb, panel_raw, panel_cc768, panel_cc192], axis=1)
        out_path = os.path.join(OUT_DIR, "%s_raw_vs_cc768_vs_cc192.png" % stem)
        cv2.imwrite(out_path, row)
        print(
            "[OK] %s  raw=%d  cc768=%d wires/%d segs  cc192=%d wires/%d segs"
            % (
                stem,
                n_raw,
                n_cc768_poly,
